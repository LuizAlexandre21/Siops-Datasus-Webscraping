    ...
    for ano in range(2013,2020):
        try:
            ...
            for k in range(0,len(Descrição)):
                sql = 'INSERT INTO Receitas_apuração_sps_estadual(estado,ano,campo,previsao_inicial,previsão_atualizada,Receitas_realizadas_Bimestre,Receitas_realizadas_Porcentagem) VALUES("{}","{}","{}","{}","{}","{}","{}")'
            ...
            var = var[1:]
            for k in range(0,len(Descrição)):
                sql = 'INSERT INTO Receitas_adicionais_financiamento_estadual(estado,ano,campo,previsao_inicial,previsão_atualizada,Receitas_realizadas_Bimestre,Receitas_realizadas_Porcentagem) VALUES("{}","{}","{}","{}","{}","{}","{}")'
                db_cur.execute(sql.format(Estado,str(ano),str(Descrição[k].replace('"',"|")),str(var[k][0]),str(var[k][1]),str(var[k][2]),str(var[k][3])))
                db_conn.commit()
                print("Insert")
            ...
            for k in range(0,len(Descrição)-1):
                sql = 'INSERT INTO Despesas_saude_natureza_estadual(estado,ano,campo,dotação_inicial,dotação_atualizada,despesas_executadas_liquidadas,despesas_executadas_inscritas,despesas_executadas_porcentagem)VALUES("{}","{}","{}","{}","{}","{}","{}","{}")'
            ...
            var=var[1:]
            for k in range(0,len(Descrição)-1):
                sql = 'INSERT INTO Despesas_saúde_não_computadas_estadual(estado,ano,campo,dotação_inicial,dotação_atualizada,despesas_executadas_liquidadas,despesas_executadas_inscritas,despesas_executadas_porcentagem)VALUES("{}","{}","{}","{}","{}","{}","{}","{}")'
                db_cur.execute(sql.format(Estado,str(ano),str(Descrição[k].replace('"',"|")),str(var[k][0]),str(var[k][1]),str(var[k][2]),str(var[k][3]),str(var[k][4])))
                db_conn.commit()
                print("Insert")
            ...
            var=var[1:]
            for k in range(0,len(Descrição)):
                sql = 'INSERT INTO Despesas_saúde_subfunção_estadual(estado,ano,campo,dotação_inicial,dotação_atualizada,despesas_executadas_liquidadas,despesas_executadas_inscritas,despesas_executadas_porcentagem)VALUES("{}","{}","{}","{}","{}","{}","{}","{}")'
                db_cur.execute(sql.format(Estado,str(ano),str(Descrição[k].replace('"',"|")),str(var[k][0]),str(var[k][1]),str(var[k][2]),str(var[k][3]),str(var[k][4])))
                db_conn.commit()
                print("Insert")
